...
// ===== בקשה חדשה מלקוח שיש לו הזמנה (דווח 19.9) =====
// "לשמור שולחן" מדליק את החיפוש, והשורה אמרה "התייחס אליה ישירות, מותר לאשר
// את הפרטים" - והבוט שפך "יש לך הזמנה ל-29.9 ב-20:00 ל-10 אנשים" בלי שנשאל.
const hNew = (requestedDateISO?: string) =>
  buildIdentityHint(RES, { channel: "whatsapp", channelUserId: "972501112222", customerText: "אני רוצה לשמור שולחן ל2 אנשים", today, windowLabel, requestedDateISO });
const dayOfA = RES.find((r) => r.id === "2")!.day!;
t("וואטסאפ מאומת - שעה וכמות רק לפי בקשה", has(hNew(), "רק אם הוא שואל"), true);
t("וואטסאפ מאומת - כבר לא 'מותר לאשר את הפרטים'", has(hNew(), "מותר לך לאשר לו את הפרטים"), false);
t("בקשה ליום אחר - אל תזכיר את הקיימת", has(hNew("2026-12-31"), "אל תזכיר את ההזמנה הקיימת"), true);
t("בקשה לאותו יום - שואל אם זו תוספת", has(hNew(dayOfA), "לאותו יום"), true);
t("בלי יום ידוע - לא להזכיר מיוזמתו", has(hNew(), "אל תזכיר אותה מיוזמתך"), true);
t("מסלול א' עם שינוי - עדיין מזהה ומתייחס", has(hA, "התייחס אליה ישירות"), true);
