...
async function phraseQuestions(
  ...
  // יותר מהמחיר.
  const model = process.env.BRAIN_REVIEW_MODEL ?? "claude-sonnet-4-6";
  ...
  await pool(batches, 6, async (batch) => {
    ...
    try {
      const resp = await client.messages.create({
        ...
        max_tokens: 2000,
        system:
          "בעל מסעדה ישראלי עובר על ההנחיות שהצטברו לצ'אטבוט שלו ומחליט מה להשאיר, מה לתקן ומה למחוק. " +
          "הוא לא מתכנת ולא ראה מעולם את הקוד. לכל הנחיה שאני נותן לך החזר שלושה שדות:\n\n" +
          "• summary - משפט אחד: מה ההנחיה הזאת גורמת לבוט לעשות מול לקוח.\n" +
          "• question - שאלה אחת קצרה בגוף שני, שהוא יכול לענות עליה מתוך היכרות עם העסק שלו " +
          '("זה עדיין המצב?", "אתה עדיין רוצה שהוא יגיד את זה ככה?", "המספר הזה נכון?").\n' +
          "• owner_decision - האם יש כאן בכלל מה להחליט לבעל העסק.\n\n" +
          "**עברית תקנית היא דרישה, לא בונוס.** הוא קורא כל שאלה כדי להחליט, ומשפט עקום עוצר אותו. " +
          "משפטים קצרים, תחביר נכון, זכר/נקבה ויחיד/רבים מתואמים, בלי תרגומית ובלי מילים מומצאות. " +
          "אם אינך מצליח לנסח שאלה ברורה על ההנחיה הזאת - זה בדרך כלל סימן ש-owner_decision הוא false.\n\n" +
          "**מתי owner_decision=false (וזה נפוץ):** כשההנחיה היא הבוט מדבר אל עצמו ולא אל הלקוח, " +
          "ולבעל העסק אין עליה שום דעה אפשרית. למשל: איך להשתמש בנתון שהמערכת מזריקה לו, מתי לקרוא לכלי, " +
          "סדר שלבים פנימי, מבנה ההודעה, עיצוב, איך לפרש שדה, מה לעשות כשמידע סותר. " +
...
/**
 * מוחק את ניסוחי השאלות כדי שינוסחו מחדש (אחרי שיפור המודל או ההנחיות).
 * **התשובות, ההערות והעריכות לא נוגעים** - הן נשמרות בנפרד ומזוהות לפי
 * מזהה הסעיף, שנגזר מהתוכן ולא מהניסוח.
 *
 * ⚠️ המספור עשוי לזוז: ניסוח מחדש מסווג מחדש גם מה טכני ולא מוצג, ושאלה
 * שיוצאת מהרשימה מזיזה את כל מי שאחריה. מי שכבר רשם הפניות למספרים צריך
 * לדעת את זה.
 */
export async function rephraseQuestions(): Promise<{ cleared: number }> {
  const stored = await loadQuestions();
  const cleared = Object.keys(stored).length;
  await saveQuestions({});
  return { cleared };
}
