...
const QUICK_MATCHERS: { key: string; patterns: RegExp[]; exclude: RegExp }[] = [
  ...
  },
  {
    ...
    // שישי הוסר מכאן (18.9): בקשת שישי חייבת קודם בירור כמה סועדים (הסף בטבלה
    // בכלל 14 - אותו סף בכל יום) - המודל מטפל, בלי להתנדב את המדיניות.
    patterns: [
...
export function sanitizeModelText(raw: string): string {
  ...
  // לוכסן אחורי לפני כוכבית/מקף ("\\*8149") - המודל בורח מהתו מדי פעם למרות
  // שהפרומפט אוסר, והלקוח רואה לוכסן. תיקון דטרמיניסטי במקום מרדף.
  t = t.replace(/\\([*\-])/g, "$1");
  ...
  // קו מפריד "---"/"___" ותווי חץ (← →) מוצגים כתווים גולמיים בכל הערוצים
  // (ממצא סריקה 6.9). מסירים בקוד - הפרומפט לבדו לא תמיד תופס.
  t = t
    .replace(/(^|\n)[ \t]*[-_]{3,}[ \t]*(?=\n|$)/g, "$1")
    .replace(/[←→⬅➡⇐⇒]/g, "")
    .replace(/\n{3,}/g, "\n\n")
 ...
 *  (בסריקה: 58% מהודעות המסנג'ר). מסירים את סימון ההדגשה בערוצים האלה בלבד,
...
function hasSecondAsk(text: string): boolean {
  // ⚠️ 14.9: הגרסה הראשונה ספרה סימני שאלה על הטקסט הגולמי, וזה הפיל תבניות על
  // התנהגות נפוצה לגמרי במסנג'ר. גל הפייסבוק חשף שני דפוסים:
  //   (א) הלקוח לוחץ פעמיים על אותו כפתור ("איפה אתם...?" פעמיים) -> שני סימני
  //       שאלה -> ההודעה נשלחה למודל למרות שיש לה תבנית מדויקת.
  //   (ב) הלקוח לוחץ על שני כפתורים שונים (שעות + מיקום) -> שוב שני סימנים,
  //       למרות ש*לשניהם* יש תבנית והם נענים במלואם בחינם.
  // נמדד על 305 שיחות אמת: 29 תורים (20% מהתורים בתשלום) נשלחו למודל לחינם.
  // התיקון: מנטרלים כפילויות, ובהודעה מרובת-שורות מתעלמים משורות שהן כשלעצמן
  // שאלת-תבנית מוכרת - הן נענות במלואן ולכן אינן "שאלה שנייה שתיפול בין הכיסאות".
  // בשורה בודדת שומרים על ההתנהגות המקורית: שם שתי שאלות באמת עלולות להיחתך.
...
function menuCategoryAnswer(raw: string, cfg: BusinessConfig): string | null {
  ...
  // "מבצע/הנחה/האפי אוור" הוחרגו (31.8): "יש מבצע על קוקטיילים?" קיבל רשימת
  // מחירים במקום את ההאפי אוור. שאלת מבצע היא שאלת מדיניות - למודל, שמכיר אותה.
  if (/גלוטן|טבעוני|צמחוני|אלרג|כשר|בלי |ללא |מחיר|עולה|ילדים|איפה|שעות|חני|הזמנ|פתוח|פירות|מבצע|הנחה|האפי|הפי ?אוור|happy/i.test(t)) return null;
    ...
    case "parking": {
      ...
      const variants = hasParkingVideo
        ? [
            ...
            "יש חניה חינמית צמודה אלינו 🙂 הכניסה אליה קצת מוסתרת, אז הנה סרטון קצר שמראה בדיוק מאיפה נכנסים.",
...
function isGateOpenRequest(text: string): boolean {
  ...
  // הודעה שכולה פועל פתיחה בציווי ("פתח" / "פתחו") - בהקשר בית הקפה אין לזה
  // משמעות אחרת, והלקוח עומד בשער וממתין. נמצא בסריקת 14.9: "פתח" לבד גלש
  // למודל (איטי ובתשלום) במקום להיפתח מיד.
  if (/^(פתח|פתחו|פתחי)[\s!.,]*$/.test(t)) return true;
  // גם המילה "שער" לבדה: בסריקת כל 5,171 השיחות היא הופיעה 36 פעם, ובכל
  // פעם הבוט פתח בפועל - אבל דרך המודל (איטי ובתשלום, והלקוח ממתין בשער).
  // 29 מתוכן הגיעו מיד אחרי שהבוט עצמו דיבר על השער, כלומר אין פרשנות אחרת.
  if (/^ה?שער[\s!.,?]*$/.test(t)) return true;
...
export async function handleIncomingMessage(
  ...
  await recordLlmUsage(result.model ?? "", result.usage ?? {}, true, "bot");
  ...
  let reply = stripBoldForChannel(sanitizeModelText(result.text ?? ""), input.channel);
