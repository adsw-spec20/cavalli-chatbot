...
const LOCATION_PATTERNS: RegExp[] = [
  ...
  /איפ[הו]\s+(אתם|נ[יי]?מצא|נמצט)/, // שגיאות כתיב נפוצות: "איפו אתם", "נימצאים"
...
const QUICK_MATCHERS: { key: string; patterns: RegExp[]; exclude: RegExp }[] = [
  { key: "location", patterns: LOCATION_PATTERNS, exclude: LOCATION_EXCLUDES },
  {
    ...
    patterns: [/^(היי|הי|שלום|אהלן|היוש|בוקר טוב|ערב טוב|צהריים טובים)[!.,\s]*$/, /^\?+$/],
    exclude: /^$/,
  },
  {
    key: "reservePolicy",
  ...
  },
  {
    ...
    patterns: [/מהי הכשרות|איזו כשרות|איזה כשרות|אתם כשרים|יש (לכם )?כשרות|תעודת כשרות|מה ההכשר|איזה הכשר|^כשרות\??$/,],
    exclude: /איפה|כתובת|שעות|תפריט|מחיר|עולה|חני|הזמנ|גלוטן|טבעוני|בשר/,
  },
  {
    key: "parking",
    // "חניה מדויקת" - לקוחות בדרך מבקשים הכוונה (מביקורת 10.8)
    patterns: [/יש (לכם )?חני(ה|יה)|איפה חונים|איפה אפשר לחנות|מה עם חני|חני(ה|יה) מדויקת|^חני(ה|יה)\??$/],
    exclude: /נכים|אופנוע|שמורה|איפה אתם|כתובת|שעות|תפריט|מחיר|עולה|הזמנ|כשר/,
  },
  {
    key: "instagram",
  ...
  {
    key: "reserveHowto",
    // "איך מזמינים" = שאלת מידע (תבנית). "אני רוצה להזמין" = כוונה אמיתית -> זרימת ההזמנה במודל
    patterns: [/איך (אפשר )?(מזמינים|להזמין) (מקום|שולחן)|איך עושים הזמנה/],
    exclude: /היום|מחר|הערב|שבת|אנשים|איש|זוג|יום הולדת|אירוע|רוצה/,
  },
  {
    key: "openNow",
    // "פתוחים עכשיו/היום" - מחושב בקוד משעון ישראל + השעות + הדריסות. חינם ומדויק.
    patterns: [/פתוחים עכשיו/, /סגורים עכשיו/, /^אתם פתוחים\??$/, /פתוחים היום/, /עד מתי (אתם )?פתוחים\??$/, /עד איזו שעה (אתם )?פתוחים\??$/, /מה השעות היום/],
 ...
 */
const ACK_RX =
  /^((אוקי|יפי|מעולה|אחלה|סבבה|רב|המון)[,!\s]+)?(תודה|תוגה|תודות|תודהה+)( רבה| רבות)?( לך| לכם| מראש)?[!.,\s🙏👍❤️🙂😊😍☕💪🥰]*$|^[🙏👍❤️😍🥰💪🙂😊❤\s]+$/;
...
function matchQuickAnswers(raw: string): string[] {
  ...
  const matchOne = (s: string): string | null => {
    const hits = QUICK_MATCHERS.filter((q) => q.patterns.some((p) => p.test(s)));
    if (hits.length !== 1) return null;
    if (hits[0].exclude.test(s)) return null;
    return hits[0].key;
  };
  ...
  switch (key) {
    ...
    case "reservePolicy":
      ...
      return (
        ...
        `- לשמירת מקום יש פיקדון של 100 ש"ח, שמשולם דרך קישור טאביט אחרי שהצוות מאשר שיש מקום.\n\n` +
    ...
    case "reserveHowto": {
      ...
      return `אפשר להזמין מקום בכמה דרכים:\n${ways}\n\nכדאי לדעת: הזמנות מראש הן לשעות הערב (מ-18:00) בימים שני-חמישי, ולשמירת מקום יש פיקדון של 100 ש"ח שמשולם דרך טאביט אחרי שהצוות מאשר שיש מקום. בימי ראשון (שנסגרים ב-18:00), בשעות היום ובימי שישי מגיעים בלי הזמנה - על בסיס מקום פנוי 🙂`;
    }
  }
  return null;
}
  ...
  try {
    ...
    if (mine.length) {
      activeReservations = mine
        .map(
          (r) =>
            `${r.status === "approved" ? "נמצא מקום (סופי אחרי תשלום הפיקדון בקישור הטאביט)" : "ממתינה לאישור הצוות"}: ${r.people} אנשים, ${r.dateText} בשעה ${r.time}, ע"ש ${r.name}${r.notes ? ` (${r.notes})` : ""}`
        )
        .join("; ");
    }
  } catch {
