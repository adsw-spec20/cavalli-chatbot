...
export function reservationSlotsHint(
  ...
  if (opts?.policyGates === false) {
    ...
    gates.push(
      `⚠️ **היום עוד לא נאמר.** ענה בחום על מה שהלקוח שאל, אבל **אסור שתופיע בתשובה הכרעת זמינות** - ` +
    ...
    gates.push(
      `⚠️ **מספר הסועדים עוד לא נאמר.** ענה על מה שהלקוח שאל עכשיו - ואם שאל אם הכמות משנה, ` +
        ...
        `לפני שתדע את המספר, וגם לא תשובה כללית עם הסתייגות ("...ואם אתם קבוצה גדולה אפשר מול ברק"). ` +
        `באותה הודעה בקש את המספר - ואם כבר שאלת קודם, נסח אחרת ותוסיף בקצרה למה זה משנה, ` +
        `אל תחזור על אותה שאלה במילים זהות.`
    );
  }
