...
const KEY = "team_questions_answers";
...
async function loadAnswers(): Promise<Record<string, TQAnswer>> {
  try {
    const raw = await getRepo().getSetting(KEY);
    return raw ? (JSON.parse(raw) as Record<string, TQAnswer>) : {};
  } catch {
    return {};
  }
}

/** תשובות "שאלות לצוות": נשמרות בשרת ומוטמעות לבוט בתהליך נפרד (עם בדיקת סתירות). */
